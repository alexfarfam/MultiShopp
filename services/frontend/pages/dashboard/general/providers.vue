<template>
    <div>
        <DataView class="!bg-transparent" :value="filteredProviderData" data-key="id">
                    ...
                    <div v-for="(item, index) in slotProps.items" :key="index">
                        <div class="flex flex-row p-2 gap-2 border border-surface-200 bg-white shadow-md rounded-xl ">
                            <div class="w-18 md:w-20 relative">
                                <i class="bg-[#001529] text-white rounded-full px-5 py-[31px] m-2 text-center !text-xl/[0] pi pi-truck"></i>
                            </div>
                                ...
                                <div class="flex flex-row md:flex-col justify-between items-start gap-3">
                                    <div>
                                        ...
                                        <div class="mt-4 md:mt-2 flex flex-col md:flex-row gap-1 md:gap-3">
                                            ...
                                            <div class="flex flex-row gap-2">
                                                <span class="font-medium text-surface-500 dark:text-surface-400 text-sm">Fecha Creación: </span> <span class="font-normal text-surface-500 dark:text-surface-400 text-sm">{{ Globalization.dateFormat2(item.date_joined, false) }}</span>
                                            </div>
                                            <div class="flex flex-row gap-2">
            ...
            <template #empty>
                ...
                <span class="mt-4 block text-2xl text-gray-400/[0.8] w-full text-center">Sin proveedores para mostrar</span>
            </template>
        </DataView>
    </div>
    ...
    <Dialog dismissable-mask v-model:visible="showEditModal" modal header="Editar Proveedor" :style="{ width: '25rem' }">
        <form @submit.prevent="confirmEdit">
            <div class="flex items-center gap-4 mb-8">
                ...
                <Checkbox v-model="providerForm.is_active" :binary="true" />
            </div>

            <div class="flex justify-end gap-2">
                <Button type="button" label="Cancelar" severity="secondary" @click="showEditModal = false"></Button>
                <Button :disabled="loadingSubmit" type="submit" label="Guardar" :loading="loadingSubmit"></Button>
            </div>
        </form>
    </Dialog>
</template>
...
<script lang="ts" setup>
    ...
    import type { ProviderItem, ProviderForm } from '~/types/dashboard/providers';
    ...
    useListen('application:search' , (message:any) => {
        ...
        filteredProviderData.value=providerData.value.filter(entry=>{
            return (
    ...
    **********************/
    ...
    const providerData=ref<ProviderItem[]>([]);
    const filteredProviderData=ref<ProviderItem[]>([]);

    const providerForm:UnwrapRef<ProviderForm> = reactive({
        is_active: false
    });
    ...
    **********************/
    const doEdit = (id:number)=>{
        currentId.value=id;
        const index = filteredProviderData.value.findIndex(obj => obj.id === currentId.value);
        const obj = filteredProviderData.value[index];
        providerForm.is_active=obj.is_active;

        showEditModal.value=true;
    };

    const confirmDelete = (id:number) => {
        const index = filteredProviderData.value.findIndex(obj => obj.id === id);
        const obj = filteredProviderData.value[index];

        confirm.require({
            message: 'Seguro(a) que quieres borrar al proveedor "'+obj.email+'"?',
            ...
            accept: async() => {
                ...
                params.append('provider_ids', JSON.stringify([id]));
                await fetchWrapper.delete('/providers/delete?'+ params).then((msg)=>{
                    filteredProviderData.value=filteredProviderData.value.filter(item => item.id !== id);
                    providerData.value=providerData.value.filter(item => item.id !== id);
    ...
    const confirmEdit = async()=>{
        const params = new URLSearchParams();
        params.append('provider_id', currentId.value.toString());

        loadingSubmit.value=true;
        await fetchWrapper.put('/providers/provider?'+params, providerForm).then((resp)=>{
            const index = filteredProviderData.value.findIndex(obj => obj.id === currentId.value);
            filteredProviderData.value[index]=resp['data'];
    ...
    **********************/
    onMounted(async()=>{
        providerData.value=await fetchWrapper.get('/providers/all');
        filteredProviderData.value=cloneDeep(providerData.value);
    });
